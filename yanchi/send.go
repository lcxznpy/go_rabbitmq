...
func NewRabbitMQDlx(exchangeName string) *Rabbit {
	rabbitMQ := NewRabbitMQ("", exchangeName, "") // 创建RabbitMQ实例
	var err error
...
// 生产者想普通交换机内发送消息
func (r Rabbit) Publish(msg string, rkey string) {
...
// 10s延迟队列
func (r Rabbit) CosumeNormal1() {
	dlxExchangeName := "yanchi_exchange" //死信交换机名称
	//1.声明正常交换机
	err := r.channel.ExchangeDeclare(
		r.Exchange, // 交换机名字
		"direct",   // 交换机类型，这里使用topic类型，即: Topics模式
	...
	argsQue["x-dead-letter-routing-key"] = "yanchi"
	//添加过期时间
	argsQue["x-message-ttl"] = 10000 //单位毫秒
	//队列最大长度
	//argsQue["x-max-length"] = 6

	//3.声明队列并添加参数
	queue, err := r.channel.QueueDeclare(
		"yanchi_normal_1", // 随机生产队列名称
		true,              // 是否持久化
		false,             // 是否自动删除
	...
	err = r.channel.QueueBind(
		queue.Name, // 队列名
		"qwer",     // 路由参数，如果匹配消息发送的时候指定的路由参数，消息就投递到当前队列（在Topics模式下，这里的key要指定）
...
func (r Rabbit) CosumeNormal2() {
	...
	argsQue["x-dead-letter-routing-key"] = "yanchi"
	//添加过期时间
	argsQue["x-message-ttl"] = 20000 //单位毫秒
	//队列最大长度
	//argsQue["x-max-length"] = 6

	//3.声明队列并添加参数
	queue, err := r.channel.QueueDeclare(
		"yanchi_normal_2", // 随机生产队列名称
		true,              // 是否持久化
		false,             // 是否自动删除
		false,             // 是否具有排他性   ,排他性连接关闭自动删除队列
		false,             // 是否阻塞处理
	...
	err = r.channel.QueueBind(
		queue.Name, // 队列名
		"asdf",     // 路由参数，如果匹配消息发送的时候指定的路由参数，消息就投递到当前队列（在Topics模式下，这里的key要指定）
		r.Exchange, // 交换机名字，需要跟消息发送端定义的交换器保持一致
		false,      // 是否阻塞处理
		nil,        // 额外的属性
	)
...
func (r Rabbit) CosumeNormal3() {
	...
	err = r.channel.QueueBind(
		queue.Name, // 队列名
		"zxcv",     // 路由参数，如果匹配消息发送的时候指定的路由参数，消息就投递到当前队列（在Topics模式下，这里的key要指定）
		r.Exchange, // 交换机名字，需要跟消息发送端定义的交换器保持一致
		false,      // 是否阻塞处理
		nil,        // 额外的属性
	)
...
func (r Rabbit) CosumeDLX() {
	...
	err = r.channel.QueueBind(
		queue.Name,        // 队列名
		"yanchi",          // 路由参数，如果匹配消息发送的时候指定的路由参数，消息就投递到当前队列（在Topics模式下，这里的key要指定）
		"yanchi_exchange", // 交换机名字，需要跟消息发送端定义的交换器保持一致
		false,             // 是否阻塞处理
		nil,               // 额外的属性
	)
...
// 设置消息过期时间，对延迟队列进行优化，这样就不需要对不同的过期时间创建不同的队列
func (r Rabbit) PublishWithTTL(msg, rkey, ttl string) {
	err := r.channel.ExchangeDeclare(
		r.Exchange, // 交换机名字
		"direct",   // 交换机类型，这里使用topic类型，即: Topics模式
		true,       // 是否持久化
	...
	err = r.channel.Publish(
		...
		amqp.Publishing{
			...
			ContentType:  "text/plain",
			Body:         []byte(msg),
			Expiration:   ttl, //设置消息过期时间，对延迟队列进行优化，这样就不需要对不同的过期时间创建不同的队列
		},
	)
	if err != nil {
		log.Println(err)
